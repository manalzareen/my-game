...
startImage=loadImage("Images/startbutton.jpg");
   ...
   endbgImg=loadImage("Images/bg2.png");
}
 ...
 if(gameState=="play2"){
    ...
    if(gameState=="END"){
      ...
      stroke("black")
            ...
            fill("orange")
            text("precation:"+precaution,270,150)
            fill("red")
  ...
  function Masks(){
    if (frameCount % 150 === 0) {
      mask = createSprite(random(windowWidth/2+600,windowHeight/2-300),10,10);
      mask.velocityY=4;
      mask.scale=0.4;
...
function Sanitizer(){
  if (frameCount % 100 === 0) {
    sanitizer= createSprite(random(windowWidth/2+80,windowHeight/2-300),100,10,10);
